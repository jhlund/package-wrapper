...
def file_hash_create_hash_file(file_path: Path, file_list: List, hash_method: str = SHA256) -> bool:
    """
...
def file_hash_check_hash_file(file_path: Path) -> Dict:
    ...
    _directory_contents = [_file for _file in _path_to_dir.glob('**/*') if _path_to_dir.joinpath(_file).is_file()]
    ...
    for line in _hash_lines:
        ...
        if line.split()[1] not in _directory_contents:
            raise FileHashE('Unable to locate the file ' + line.split()[1]
                            + ' in the compressed archive')

        for file in _directory_contents:
            if line.split()[1] == file:
                result[file] = file_hash_check(_path_to_dir.joinpath(Path(file)), line.split()[0])
